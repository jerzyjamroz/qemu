...
#ifdef DEBUG_LVL
#define DPRINTK(LVL, FMT, ...) do{if((LVL)<=DEBUG_LVL) printf(TYPE_MPC8540_I2C " : " FMT, ## __VA_ARGS__); }while(0)
#else
#define DPRINTK(LVL, FMT, ...) do{}while(0)
#endif

#define LOG(MSK, FMT, ...) qemu_log_mask(MSK, TYPE_MPC8540_I2C " : " FMT, ## __VA_ARGS__)
...
uint64_t mpc8540_i2c_read(void *opaque, hwaddr addr, unsigned size)
{
    ...
    case 0xc: /* STATUS */
        ...
        break;
    case 0x10: /* DATA */
        /* Reads are "pipelined" and so return the previous value of the register... */
        val = i2c->dbuf;
