...
#ifdef DEBUG_NVIC
#define DPRINTF(LVL, fmt, ...) DPRINTFx(LVL, "armv7m_nvic: " fmt, ## __VA_ARGS__)
#define DPRINTFx(LVL, fmt, ...) \
do { if ((LVL) <= DEBUG_NVIC) { \
    fprintf(stderr, fmt , ## __VA_ARGS__); \
} } while (0)
#else
#define DPRINTF(LVL, fmt, ...) do {} while (0)
#define DPRINTFx(LVL, fmt, ...) do {} while (0)
#endif
...
struct VecInfo {
    ...
    int16_t prio_group; /* group priority [-2, 0xff] */
...
{
    unsigned submask = (1<<(s->cpu->env.v7m.prigroup+1))-1;
...
void armv7m_nvic_set_pending(void *opaque, int irq)
{
            ...
            && irq != ARMV7M_EXCP_NMI)
    {
        ...
        if (escalate) {
            ...
            s->cpu->env.v7m.hfsr |= HFSR_FORCED;
    ...
    DPRINTF(0, "Pending %d%s at %d%s running %d\n",
            irq, vec->enabled ? "" : " (disabled)",
            vec->prio_group, env->v7m.pending == irq ? " (highest)" : "",
...
static uint32_t nvic_readl(NVICState *s, uint32_t offset)
{
    ...
    case 0xd0c: /* Application Interrupt/Reset Control.  */
        return 0xfa050000 | (s->cpu->env.v7m.prigroup<<8);
...
static void nvic_writel(NVICState *s, uint32_t offset, uint32_t value)
{
    ...
    case 0xd0c: /* Application Interrupt/Reset Control.  */
        if ((value >> 16) == 0x05fa) {
            ...
            if (value & 0x700) {
                unsigned i, submask;
                DPRINTF(0, "Change PRIGROUP 0x%x -> ",
                        s->cpu->env.v7m.prigroup);
                s->cpu->env.v7m.prigroup = (value>>8) & 0xf;
                DPRINTF(0, "0x%x\n",
                        s->cpu->env.v7m.prigroup);
                submask = (1<<(s->cpu->env.v7m.prigroup+1))-1;
                s->cpu->env.v7m.basepri &= ~submask;
    ...
    case 0xdb4: /* MPU_RBAR_A3 */
    {
        uint32_t range = cpu->env.cp15.c6_rgnr;
        uint32_t base = value;
    ...
    case 0xdb8: /* MPU_RASR_A3 */
    {
        unsigned range = cpu->env.cp15.c6_rgnr;
                              ...
                              uint64_t value, unsigned size)
{
    ...
    case 0x100 ... 0x13c: /* NVIC Set enable */
        ...
        /* fall through */
    case 0x180 ... 0x1bc: /* NVIC Clear enable */
        offset = 8*(offset-0x180)+16; /* vector # */

        DPRINTF(0, "IRQ %sable ", setval ? "en" : "dis");
        for (i = 0, end = size*8; i < end && offset+i < s->num_irq; i++) {
            if (value & (1<<i)) {
                DPRINTFx(0, "%u, ", offset+i);
    ...
    case 0x200 ... 0x23c: /* NVIC Set pend */
        ...
        /* fall through */
    case 0x280 ... 0x2bc: /* NVIC Clear pend */
        offset = 8*(offset-0x280)+16; /* vector # */
    ...
    case 0x300 ... 0x37c: /* NVIC Active */
        return; /* R/O */
    case 0x400 ... 0x7ec: /* NVIC Priority */
        offset = 8*(offset-0x400)+16; /* vector # */
...
static const VMStateDescription vmstate_VecInfo = {
    ...
    .fields = (VMStateField[]) {
        ...
        VMSTATE_INT16(prio_group, VecInfo),
