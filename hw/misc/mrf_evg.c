...
#ifdef MRF_DEBUG
#define	DBGOUT(LVL,fmt, ...) do{if(MRF_DEBUG>=(LVL)) fprintf(stderr, "mrf-evg: %s() " fmt, __func__, ## __VA_ARGS__); } while(0)
#else
...
void mrf_evg_update(EVGState *s)
{
...

    {
        ...
        DBGOUT(3,"IRQ %d Flag %08x Act %08x Ena %08x\n", level,
...
void evg_dbuf_send(EVGState *d)
{
    ...
    if((ctrl&0x00030000)!=0x00030000) {
        ERR(LOG_GUEST_ERROR,"DBuf can't send when not enabled 0x%08x\n",(unsigned)ctrl);
        return;
    }

    DBGOUT(2,"DBuf send %u bytes\n", 4*N);
                 ...
                 unsigned size)
{
    ...
    DBGOUT(2,"write "TARGET_FMT_plx" = %08x\n", addr, (unsigned)val);
