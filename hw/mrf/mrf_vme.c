...
#ifdef MRF_DEBUG
#define	DBGOUT(fmt, ...) fprintf(stderr, TYPE_MRF_VME ": %s() " fmt, __func__, ## __VA_ARGS__)
#else
#define	DBGOUT(fmt, ...) do {} while (0)
#endif

#define ERR(mask, fmt, ...)  qemu_log_mask(mask, TYPE_MRF_VME ": " fmt, ## __VA_ARGS__)
...
static void mrf_csr_update(MRFVMEState *d)
{
    ...
    for(i=0; i<3; i++) {
        ...
        if(amod) {
            DBGOUT("Map FN[%u] to %02x @%08x\n", i, amod, (unsigned)base);
...
mrf_csr_read(void *opaque, hwaddr rawaddr, unsigned size)
{
    ...
    DBGOUT("CSR read "TARGET_FMT_plx" -> %02x\n", rawaddr, (unsigned)ret);
    return ret;
}
                 ...
                 unsigned size)
{
    ...
    DBGOUT("CSR write "TARGET_FMT_plx" <- %02x\n", rawaddr, (unsigned)val);
    ...
    case UCSR_IRQ_LEVEL: /* MRF IRQ level */
        ...
        break;
    default:
        ERR(LOG_UNIMP|LOG_GUEST_ERROR, "Write to unsupported CSR/CR 0x%06x\n",(unsigned)addr);
        return;
    }
...
static void mrf_vme_realize(struct VMEDevice *dev, Error **errp)
{
    ...
    DBGOUT("core is %s\n", object_get_canonical_path(OBJECT(coredev)));
