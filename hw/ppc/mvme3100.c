...
#define WRITE(N, BUF)  \
     ...
     */
    at24c_eeprom_write(dev, 0x10f8, sizeof(gev), gev);
...
static void mvme3100_init(MachineState *machine)
{
    ...
    PPCE500Params params = {
         ...
         *
         * Note that this number reflects whatever motload would do before boot.
         * This involves the time base control bits in the HID0 register,
         * which are not currently modeled, and may use a board specific
         * clock.
         */
    ...
    object_property_add_child(qdev_get_machine(), "cpld",
                              &cpld->parent_obj.parent_obj, &error_fatal);
    qdev_init_nofail(&cpld->parent_obj);
    ...
    if(object_class_by_name("tsi148")) {
        ...
        pdev = pci_create_multifunction(pcibus, PCI_DEVFN(0x11,0), false, "tsi148");
        dev = DEVICE(pdev);
        ...
        printf("Unable to add tsi148 vme bridge (qemu built w/o CONFIG_TSI148)\n");
    }
    ...
    object_property_add_child(qdev_get_machine(), "vpd", &dev->parent_obj,
                              &error_fatal);
    ...
    if (bios_name) {
         ...
         */
        int bsize = get_image_size(bios_name);
        if (bsize != 8*1024*1024 ||
                -1 == load_image_targphys(bios_name,
                                          0xff800000, 8*1024*1024))
        {
            fprintf(stderr, "qemu: could not load bios file '%s' (%u bytes)\n",
                    bios_name, (unsigned)bsize);
            exit(1);
