...
void append_vpd(vpdeeprom *vpd, uint8_t id, size_t cnt, const void *val)
{
    ...
    if ((id == 0xff && remaining < 2)           /* 2 bytes for final entry */
       || (remaining + 4 < cnt || cnt > 255)) { /* space for this entry
                                                 *  and 2 bytes for the final
                                                 */
        error_report("VPD overflow");
        return;
    }
...
static
void build_vpd(const MVME3100Class *m3c, char *buf, size_t cnt,
               const char *extra)
{
    MachineClass *mc = MACHINE_CLASS(m3c);
    ...
    append_string_vpd(&vpd, 1, mc->desc);

    /* serial number */
    append_string_vpd(&vpd, 3, "E0120000");

    /* CPU Freq. */
    append_u32_vpd(&vpd, 5, m3c->cpu_freq);
    ...
    if (vpd.cur - vpd.base > 0x10f8) {
        error_report("VPD overflows GEV area.");
        return;
    }
    ...
    if (extra) {
        ...
        for (i = 0; opts[i]; i++) {
            ...
            if (olen == 0) {
                ...
                error_report("Missing '=' in -append %s", extra);
                continue;
            }
...
static void mvme3100_cpu_reset(void *opaque)
{
     ...
     * HID0[TBEN]=1, HID0[SEL_TBCLK]=0
     */
    env->spr[SPR_HID0] |= HID0_TBEN;
...
static void mvme3100_init(MachineState *machine)
{
    ...
    MVME3100Class *m3c = MVME3100_GET_CLASS(machine);
    DeviceState *dev;
    BusState *i2c;
    PCIBus *pci0, *pci1;
    ...
    qdev_prop_set_uint32(dev, "porpllsr", m3c->porpllsr);
    ...
    {
        DriveInfo *drvinfo = drive_get(IF_PFLASH, 0, 0);
        if (drvinfo) {
...

    {
        ...
        build_vpd(m3c, buf, 8192 * 8, machine->kernel_cmdline);
    ...
    if (!qtest_enabled()) {
        ...
        if (!fullname) {
            error_report("qemu: could not find bios file '%s'",
                         bios_name);
            exit(1);
        }
        ...
        {
            error_report("qemu: could not load bios file '%s'"
...

    {
        ...
        if (machine->kernel_filename &&
                -1 == image_size)
        {
            error_report("qemu: could not load file '%s'",
        ...
        {
            error_report("qemu: entry-address out of range");
            exit(1);
        }
...
static void mvme3100_inst_init(Object *obj)
{
    ...
    /* set defaults.  Override with -machine */
    mvme3100->load_address = 0x10000;
    /* entry==0 uses load_address */
                                ...
                                Error **errp)
{
    ...
    if (strcmp(name, "load-address") == 0) {
        ...
        error_report("logic error: mvme3100 has no prop '%s'", name);
        exit(1);
    }
...
static void mvme3100_machine_class_init(ObjectClass *klass, void *raw)
{
...
static const TypeInfo mvme3100_type = {
    ...
    .class_init = mvme3100_machine_class_init,
    .class_size = sizeof(MVME3100Class),
};

static void mvme3100_1152_machine_class_init(ObjectClass *klass, void *raw)
{
    MachineClass *mc = MACHINE_CLASS(klass);
    MVME3100Class *m3c = MVME3100_CLASS(klass);

    /* also used in VPD ROM */
    mc->desc = "MVME3100-1152";
    mc->default_ram_size = 256 * (1 << 20);
    m3c->cpu_freq = 666666666u;
    /* CCB/PCI  -> 5/1
     * core/CCB -> 2/1
     *
     ...
     */
    m3c->porpllsr = 0x0004000a;
}

static const TypeInfo mvme3100_1152_type = {
    .name = MACHINE_TYPE_NAME("mvme3100-1152"),
    .parent = TYPE_MVME3100,
    .class_init = mvme3100_1152_machine_class_init,
};

static void mvme3100_1263_machine_class_init(ObjectClass *klass, void *raw)
{
    MachineClass *mc = MACHINE_CLASS(klass);
    MVME3100Class *m3c = MVME3100_CLASS(klass);

    /* also used in VPD ROM */
    mc->desc = "MVME3100-1263";
    mc->default_ram_size = 512 * (1 << 20);
    m3c->cpu_freq = 833333333u;
    /* CCB/PCI  -> 5/1
     * core/CCB -> 5/2
     */
    m3c->porpllsr = 0x0005000a;
}

static const TypeInfo mvme3100_1263_type = {
    .name = MACHINE_TYPE_NAME("mvme3100-1263"),
    .parent = TYPE_MVME3100,
    .class_init = mvme3100_1263_machine_class_init,
};
