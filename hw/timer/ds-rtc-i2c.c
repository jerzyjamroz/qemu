...
void dsrtc_latch(DSRTCState *ds)
{
    struct tm now;
    bool use12;
    int wday_offset = ds->wday_offset;

    qemu_get_timedate(&now, ds->time_offset, &wday_offset);
...
void dsrtc_update(DSRTCState *ds)
{
    ...
    ds->time_offset = qemu_timedate_diff(&now, &wday_offset);
    ds->wday_offset = wday_offset;
