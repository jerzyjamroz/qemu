 ...
 *
 * As the variable sized windows can be configured to open into
 * various VME address spaces, and alias regions can't be easily
 * switched to alias a different region, the outbound (PCI -> VME)
 * windows use the IOMMU interface.
 *
...
void out_window_setup(TSI148State *tsi, unsigned N)
{
    ...
    uint64_t pci_start = tsi_read64(tsi, R_OTSAU(N)),
             ...
             vme_start;
    uint32_t Wstat = tsi->REG(R_OTAT(N));
    bool enabled = !!(tsi->enable && (Wstat & (1u<<31))),
    ...
    if (changed) {
        DBGOUT("Window %u enable start=%08x end=%08x offset=%08x amod=%s\n",
               N, (unsigned)pci_start, (unsigned)pci_end, (unsigned)vme_start, vmespace->name);
...
void crg_window(TSI148State *tsi)
{
    MemoryRegion *reg;
    uint8_t amod = 0;
    uint32_t attrs = tsi->REG(R_CRGAT);
    hwaddr off = tsi->REG(R_CBAU);
    bool enabled = !!(attrs & (1 << 7));
    ...
    /* TODO, can respond to both SUP and PRG, only SUP atm. */
    ...
    DBGOUT("CRG window enabled %s # %08x\n", reg->name, (unsigned)off);
}
...
void do_dma(TSI148State *tsi, unsigned eng)
{
    ...
    if (ctrl & (1 << 23)) { /* Direct DMA */
        printf(" Direct\n");
        D.src = DMA_GET64(eng, 0x520);
        D.dst = DMA_GET64(eng, 0x528);
...
uint64_t tsi148_iack_read(void *opaque, hwaddr addr, unsigned size)
{
    ...
    if (size > 1 || (offset & 0x3)) {
        LOG(LOG_UNIMP, "iack read%u not implemented.  Treat as read8\n",
            size*8);
        size = 1;
    }
...
static void tsi148_realize(PCIDevice *pci_dev, Error **errp)
{
    ...
    memory_region_init_alias(&tsi->crg_crcsr, obj, "tsi148-crg-image", &tsi->crg,
                             0, CRG_SIZE);
    /* initially un-mapped */
...
static void tsi148_reset(DeviceState *dev)
{
    ...
    tsi->reg[R_VMCTRL >> 2] = 3; /* VME Master Control */
    tsi->reg[R_VSTAT >> 2] = 0x5900 | tsi->slot; /* TODO GAP */
    tsi->reg[0xff4 >> 2] = 0x10; /* Board Fail */
    tsi->reg[R_CBAR >> 2] = tsi->slot << 3;

    /* unmap and disable movable MemoryRegions */

    if (memory_region_is_mapped(&tsi->crg_crcsr)) {
        vme_del_region(&tsi->vdev, VME_AM_CRCSR, &tsi->crg_crcsr);
    }
