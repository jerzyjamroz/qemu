...
#if 1
#define	DBGOUT(fmt, ...) fprintf(stderr, TYPE_VME_BUS": " fmt, ## __VA_ARGS__)
#else
...
{
    /* here live weird rules for mapping address modifiers
     * to address spaces.
     */
...
void vme_add_region(VMEDevice *dev, uint8_t amod, hwaddr base, MemoryRegion *reg)
{
    VMEBus *bus = dev->bus;
...
static
void vme_bus_realize(BusState *bus, Error **errp)
{
    VMEBus *vme = VME_BUS(bus);
    (void)errp;

    DBGOUT("bus realize %p\n", vme);
}
...
void vme_bus_inst_init(Object *obj)
{
    ...
    for(i=0, cur=vmaps; cur->name; i++, cur++) {
        ...
        space->bus = vme;
        space->amod = cur->amod;
        memory_region_init_io(&space->root, obj, &berr_ops, space, cur->name, cur->size);
...
void vme_bus_class_init(ObjectClass *klass, void *data)
{
    ...
    //VMEBusClass *vc = VME_BUS_CLASS(klass);

    bc->realize = &vme_bus_realize;
